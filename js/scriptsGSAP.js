...
const populateList = (items = [], list, regEx = "") => {
  const tasksHtml = items.map((item, index) => {
    return {
      html: `<li data-index="${index}">
              ...
              <button class="check ${
                item.check ? "checked" : ""
              }" data-index="${index}"><span></span></button>
  ...
  list.innerHTML = tasksHtml
    .filter(item => item.text.toUpperCase().includes(regEx))
    .map(item => item.html)
    .join("");
};
...
const updateTasksNumber = (el, num) => {
  ...
  changeNumAnim
    ...
    .to(el, 0.4, { rotationX: 180, opacity: 0 })
    .add(() => (el.textContent = num))
    .to(el, 0.4, { rotationX: 360, opacity: 1 });
};
...
const activeAddAnimation = () => {
  ...
  addAnim
    .set(
      lastEl,
      { transformPerspective: "600", y: "-=100%", z: -100, opacity: 0 },
      0
    )
    .to(
      lastEl,
      0.6,
      { y: "+=100%", z: 0, opacity: 1, ease: Back.easeOut.config(3) },
      "-=.2"
    );
};
...
const activDeleteAnimation = (currentEl, index) => {
  ...
  delAnim
    .to(
      currentEl,
      0.5,
      { opacity: 0, scale: 0, ease: Back.easeIn.config(2) },
      "deleteTask"
    )
    .to(
      slideUpTasks,
      0.4,
      { y: "-=100%", ease: Back.easeIn.config(2) },
      "deleteTask+=.2"
    );
...
const searchTasks = e => {
  ...
  const numTasks = tasks.filter((task, index) =>
    task.text.toUpperCase().includes(reg)
  ).length;
  ...
  searchAnim
    .to(list, 0.3, { opacity: 0, scale: 0, ease: Power2.easeIn })
    .addCallback(() => populateList(tasks, list, reg))
    .to(list, 0.3, { opacity: 1, scale: 1, ease: Back.easeOut.config(1) });
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onLoadAnim
    ...
    .set("ul > li", { opacity: 0 })
    .staggerTo(".wrapper, header", 0.4, { y: 0, opacity: 1 }, 0.2)
    .addLabel("items", "+=.2")
    .staggerFromTo(
      "ul>li",
      0.5,
      { cycle: { x: [-600, 600], rotationZ: [20, -20] }, y: 40 },
      { opacity: 1, x: 0, rotationZ: 0, y: 0, cycle: {} },
      0.1,
      "items"
    )
    .staggerFromTo(
      ...
      0.5,
      { scale: 0 },
      { scale: 1, ease: Back.easeOut.config(1, 0.4) },
      0.1,
      "items+=.2"
    )
    .staggerFromTo(
    ...
    .to("footer", 1, { x: "0%", ease: Back.easeOut.config(2, 0.4) })
    .timeScale(1.5);
});
